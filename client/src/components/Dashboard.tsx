...
export default function Dashboard() {
  ...
  const computeStatusCounts = useCallback((jobs: Job[]) => {
    ...
    for (const j of jobs) {
      ...
      if (j.email_sent) { counts.email_sent++; continue; }
      if (isBadData(j) && st !== "deleted") { counts.ignored++; continue; }
  ...
  const applyFilter = useCallback((results: Job[], filter: string) => {
    return results.filter(j => {
      ...
      if (j.email_sent) return filter === "email_sent" || filter === "all";
  ...
  const handleDelete = async (id: string) => {
    ...
    const updated = allResults.map(j => j.id === id ? { ...j, status: "deleted", deleted_at: new Date().toISOString() } : j);
    setAllResults(updated);
    setFilteredResults(applyFilter(updated, activeFilter));
    setStatusCounts(computeStatusCounts(updated));
